...
implement. Search `fengshui-shifu-api/ai/plans/` and
`fengshui-shifu-ui/ai/plans/` for a matching file.
...
Check the current branch in each repo the plan touches — `git -C <repo> branch
--show-current`. This is read-only and fine to run.
...
For a cross-cutting feature the repos are checked out independently, so verify
each one separately; they can easily be on different branches.
...
- `fengshui-shifu-ui`: **no test runner or linter exists.** Run
  `npx tsc --noEmit` and `npm run build:web`, and state plainly what each does
  and does not check — `build:web` bundles through Babel and strips types
  without checking them, so it is not a typecheck.

Never report a checkpoint as done on a red or skipped gate. If a gate cannot be
run, say so explicitly rather than omitting it.
...
entry in whichever repo(s) changed. If no changelog file exists yet, do not
invent the convention silently — ask whether to start one.

If the API/UI contract changed, confirm `fengshui-shifu-ui/src/services/api.ts`
was updated to match; per `CLAUDE.md` this is not automatic.

## 6. Manual Test Instructions (Phase 6)

Give concrete, copy-pasteable steps, per repo, covering the plan's Test Plan
cases. Be explicit about what you verified yourself and what you could not —
never imply a visual check happened if no one looked at the screen.

## 7. Stop

Summarize, then hand off to `/commit-message` for the final message(s), summary
and PR description. Do not run it automatically.
